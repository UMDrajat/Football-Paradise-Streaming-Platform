...
def streaming_page_default(request):
    """
    Renders the default streaming page for the static RTMP URL.
    """
    stream_url = "http://localhost/hls/test.m3u8"  # Updated to match HLS setup
    return render(request, "streaming/streaming_page.html", {"stream_url": stream_url})


def streaming_page(request, stream_key):
    """
    Dynamically renders the streaming page for a specific stream key.
    """
    stream_url = f"http://localhost/hls/{stream_key}.m3u8"  # Correct HLS playlist URL
    return render(request, "streaming/streaming_page.html", {"stream_url": stream_url})
...
def is_upload_allowed(user):
    # Allow only staff users or users in a specific group 
    return user.is_authenticated and user.is_staff 

@user_passes_test(is_upload_allowed)
