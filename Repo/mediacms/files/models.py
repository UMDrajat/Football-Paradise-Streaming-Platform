...
def original_media_file_path(instance, filename):
    ...
    # Check if user is set, otherwise use a default folder name like "anonymous"
    username = instance.user.username if instance.user else "anonymous"
    
    return f"{settings.MEDIA_UPLOAD_DIR}user/{username}/{file_name}"
...
class Media(models.Model):
    ...
    user = models.ForeignKey("users.User", on_delete=models.CASCADE, help_text="user that uploads the media", null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.user:
            # Optionally assign a default user here or handle missing user gracefully
            raise ValueError("User must be assigned before saving.")
